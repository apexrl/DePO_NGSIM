...
MAX_SPEED = 130
dir_prefix = str(Path(__file__).parent)
        ...
        for cars in state:
            if cars:
                ...
                if behind:
                    ...
                    if (
                        d[0] < -alpha
                        and abs(d[1]) + alpha < (self._width + behind._width) / 2
                    ):
                ...
                if ahead:
                    ...
                    if (
                        d[0] < -alpha
                        and abs(d[1]) + alpha < (self._width + ahead._width) / 2
                    ):
                        self.collisions_per_frame += 1
...
class I80(Simulator):
    ...
    def __init__(self, **kwargs):
        ...
        self.max_frame = -1
        pth = dir_prefix + "/traffic-data/state-action-cost/data_i80_v0/data_stats.pth"
        self.data_stats = (
    ...
    def _get_data_frame(self, time_slot, x_max, x_offset):
        ...
        file_name = dir_prefix + f"/traffic-data/xy-trajectories/{time_slot}"
    ...
    def reset(self, frame=None, time_slot=None, vehicle_id=None, train_only=False):
        ...
        if self.vehicle_ids is not None:
            ################################################################################
            # Looping over specified vehicle ids ONLY
            ################################################################################
            assert not (
                frame or time_slot or vehicle_id
            ), "Already selecting training episode from file."
            time_slot, vehicle_id = self.vehicle_ids[
                self.episode % len(self.vehicle_ids)
            ].copy()
            # self.episode will be self-increased in super().reset()
            ################################################################################
    ...
    def step(self, policy_action=None):
        ...
        if self.normalise_action and policy_action is not None:
            np.multiply(policy_action, self.data_stats["a_scale"], policy_action)
        ...
        if self.controlled_car and self.controlled_car["locked"]:
            ...
            if return_:
                if self.store_sim_video and return_[-2]:
                    assert self.video_save_dir is not None
                    self.controlled_car["locked"].dump_state_image(
                        self.video_save_dir, "video"
                    )
                return return_
