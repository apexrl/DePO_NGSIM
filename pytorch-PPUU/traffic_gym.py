...
class Car:
    ...
    def draw(self, surface, mode="human", offset=0):
        ...
        if mode == "human":
            ...
            return _r
        if mode == "machine":
            return draw_rect(surface, colours["b"], rectangle, d, bounding_box=True)
        if mode == "ego-car":
            return draw_rect(
                surface,
                colours["r"],
                ...
                d,
                bounding_box=True,
                surrounding_circle=True,
            )
        if mode == "ghost":
            return draw_rect(surface, colours["r"], rectangle, d)

    def move(self, target_pos):
        """
        Update current position to the target_pos
        """
    ...
    def step(
        ...
        if not self.is_controlled or self.clip_action:
        ...
        self._direction = direction_vector / (np.linalg.norm(direction_vector) + 1e-6)
    ...
    def store(self, object_name, object_):
        if object_name == "action":
            ...
            self._actions = torch.Tensor(object_)
        elif object_name == "state":
            # self._states.append(self._get_obs(*object_))
            self._states = self._get_obs(*object_)
    ...
    def get_last(self, done, norm_state=False, return_reward=False, gamma=0.99):
        ...
        # NOTE(zbzhu): Get ego and neighbors
        ...
        observation = dict(context=state_images, state=states.reshape(1, -1))

        cost = dict(
            proximity_cost=self._states[2],
            # lane_cost=self._states_image[1],
            # pixel_proximity_cost=self._states_image[2],
            lane_cost=0.0,
        ...
        if (
            ...
            observation = states.reshape(-1).numpy()
    ...
    def dump_state_image(self, save_dir="scratch/", mode="img"):
        ...
        if mode == "video":
            save_dir = os.path.join(save_dir, "{}.mp4".format(self.id))
...
class Simulator(core.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=-1e5, high=1e5, shape=(7 * 4,), dtype=np.float32
        )
    ...
    def step(self, policy_action=None):
        ...
        for v in self.vehicles[:]:
            ...
            for lane in range(self.nb_lanes):
                if lane in lanes_occupied and v not in self.lane_occupancy[lane]:
                    # Enter lane
                    bisect.insort(self.lane_occupancy[lane], v)
                elif lane not in lanes_occupied and v in self.lane_occupancy[lane]:
                    # Leave lane
                    self.lane_occupancy[lane].remove(v)
            # Remove from the environment cars outside the screen
            if v.back[0] > self.screen_size[0]:
                for lane in lanes_occupied:
                    self.lane_occupancy[lane].remove(v)
        ...
        if (
            ...
            * self.delta_t
            ...
            if free_lanes:
                ...
                for lane in car.get_lane_set(self.lanes):
                    # Prepend the new car to each lane it can be found
                    self.lane_occupancy[lane].insert(0, car)
        ...
        for v in self.vehicles:
            ...
            if v.is_controlled and policy_action is not None:
                ...
                action = v.policy_hardcoded(state)
            ...
            for i, v in enumerate(self.vehicles):
                if (self.store or v.is_controlled) and v.valid:
                    ...
                    if self.store_sim_video:
                        ...
                        v.draw(vehicle_surface, mode="ego-car", offset=max_extension)
                        v.frames.append(
