...
def proximity_cost(
    ...
    images = images.view(bsize, npred, nchannels, crop_h, crop_w)
    costs = torch.max(
        (proximity_mask * images[:, :, green_channel].float()).view(bsize, npred, -1), 2
    )[0]
...
def save_movie(
    ...
    for t in range(images.shape[0]):
        img = images[t]
        img = np.concatenate((img, np.zeros((24, 24, 3)).astype("float")), axis=0)
        img = np.uint8(img)
...
def embed(Z, ztop, ndim=3):
    ...
    Z_all = np.concatenate((ztop, Z), axis=0)
...
def parse_command_line(parser=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="coefficient of uncertainty regularization term",
    )
    parser.add_argument("-u_hinge", type=float, default=0.5)
    parser.add_argument(
        "-lambda_a", type=float, default=0.0, help="l2 regularization on actions"
    )
    parser.add_argument(
        "-lambda_l", type=float, default=0.2, help="coefficient of lane cost"
    )
    parser.add_argument(
        "-lambda_o", type=float, default=1.0, help="coefficient of offroad cost"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=m3,
        help="dynamics model used to train the policy network",
    )
    ...
    parser.add_argument(
        "-tensorboard_dir",
        type=str,
        default="models/policy_networks",
    ...
    if tensorboard_enabled:
        ...
        if hasattr(opt, "model_file"):
            ...
            raise AttributeError(
                "options doesn't contain neither model_file nor mfile field"
            )
        script_name = os.path.splitext(sys.argv[0])[0]
        tensorboard_log_dir = os.path.join(
            opt.tensorboard_dir, f"tb_log_{script_name}_{model_name}_{date_str}"
        )
...
def denormalise_state(state, model_stats):
    return (
        (model_stats["s_mean"] + model_stats["s_std"] * state.detach().cpu())[:, :, 2:]
        / 24
        * 3.7
        * 3.6
    )
...
def are_polygons_intersecting(poly_a, poly_b) -> bool:
    ...
    :param poly_a: polygon A, as a list of [x, y] points
    :param poly_b: polygon B, as a list of [x, y] points
    :return: are intersecting
    """
    intersecting = True
    for polygon in [poly_a, poly_b]:
        for p1, p2 in zip(polygon[:-1], polygon[1:]):
            ...
            min_a, max_a = project_polygon(poly_a, normal)
            min_b, max_b = project_polygon(poly_b, normal)
...
# def check_collisions_with_others()
