...
class PathBuilder:
    ...
    def __len__(self):
        return max(
            [len(path_builder) for path_builder in self.agent_path_builders.values()]
        )
    ...
    def get_all_agent_dict(self, key):
        return {
            a_id: self.agent_path_builders[a_id][key]
            for a_id in self.agent_ids
            if key in self.agent_path_builders[a_id]
        }

    def non_empty_keys(self):
        return [
            a_id for a_id in self.agent_ids if len(self.agent_path_builders[a_id]) > 0
        ]
