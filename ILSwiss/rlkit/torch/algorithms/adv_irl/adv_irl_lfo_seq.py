...
from rlkit.torch.algorithms.torch_base_seq_algorithm import TorchBaseAlgorithm
from rlkit.data_management.path_builder import PathBuilder

from tqdm import tqdm


class AdvIRL_LfO_Seq(TorchBaseAlgorithm):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pred_observations_n = {
            t: np.array([None for _ in range(self.training_env_num[t])])
    ...
    def _do_training(self, epoch):
        if (
            ...
            self._do_inverse_dynamic_training(epoch, self.current_type, False)

        for t in range(self.num_update_loops_per_train_call):
            for _ in range(self.num_disc_updates_per_loop_iter):
                self._do_reward_training(epoch, self.current_type)

            for _ in range(self.num_policy_updates_per_loop_iter):
                self._do_policy_training(epoch, self.current_type)

        self.type_itr = (self.type_itr + 1) % len(self.possible_types)
        self.current_type = self.possible_types[self.type_itr]
        ...
        if path_builder:
            ...
            for a_id in observation_n.keys():
                ...
                if a_id not in next_observation_n.keys():
                    continue
                self._current_path_builder[self.current_type][env_id][a_id].add_all(
    ...
    def start_training(self, start_epoch=0):
        ...
        self._current_path_builder = {
            t: [PathBuilder(self.agent_ids) for _ in range(self.training_env_num[t])]
            for t in self.possible_types
        }
        ...
        for epoch in gt.timed_for(
            ...
            for steps_this_epoch in tqdm(
                range(
                    ...
                    // self.training_env_wait_num[self.current_type]
                ),
                unit_scale=self.training_env_wait_num[self.current_type],
            ):
                try:
                    (
                        self.pred_observations_n[self.current_type][
                            self._ready_env_ids[self.current_type]
                        ],
                        self.actions_n[self.current_type][
                            self._ready_env_ids[self.current_type]
                        ],
                    ) = self._get_action_and_info(
                        self.observations_n[self.current_type][
                            self._ready_env_ids[self.current_type]
                        ]
                    )
                except IndexError as e:
                    print(type(self.observations_n))
                    print(type(self.observations_n[self.current_type]))
                    raise e

                for action_n in self.actions_n[self.current_type][
                    self._ready_env_ids[self.current_type]
                ]:
                ...
                if self.render:
                    self.training_env[self.current_type].render()

                (next_obs_n, rewards_n, terminals_n, env_infos_n,) = self.training_env[
                    self.current_type
                ].step(
                    self.actions_n[self.current_type][
                        self._ready_env_ids[self.current_type]
                ...
                self._ready_env_ids[self.current_type] = np.array(
                    [i["env_id"] for i in env_infos_n]
                )
                ...
                self._n_env_steps_total += self.training_env_wait_num[self.current_type]
                ...
                self._handle_vec_step(
                    ...
                    terminals_n,
                    env_ids=self._ready_env_ids[self.current_type],
                ...
                if np.any(terminals_all):
                    end_env_id = self._ready_env_ids[self.current_type][
                        np.where(terminals_all)[0]
                    ]
                    self._handle_vec_rollout_ending(end_env_id)
                    if not self.training_env[self.current_type].auto_reset:
                        self.observations_n[self.current_type][
                            end_env_id
                        ] = self.training_env[self.current_type].reset(end_env_id)

                elif np.any(
                    np.array(
                        [
                            len(self._current_path_builder[self.current_type][i])
                            for i in range(len(self._ready_env_ids[self.current_type]))
                        ]
                    )
                    ...
                    env_ind_local = np.where(
                        np.array(
                            [
                                len(self._current_path_builder[self.current_type][i])
                                for i in range(
                                    len(self._ready_env_ids[self.current_type])
                                )
                            ]
                        )
                        >= self.max_path_length
                    ...
                    self.observations_n[self.current_type][
                        env_ind_local
                    ] = self.training_env[self.current_type].reset(env_ind_local)
