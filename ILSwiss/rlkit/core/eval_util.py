...
def get_generic_path_information(
    paths, env, stat_prefix="", dpo=False, ego_obs_idx_n=None
):
    """
    Get an OrderedDict with a bunch of statistic names and values.
    """
    agent_ids = set.union(*map(set, [path.non_empty_keys() for path in paths]))
    ...
    if hasattr(env, "get_unscaled_obs"):
        distance_travelled_n = {
            a_id: [
                ...
                for path in paths
                if a_id in path.non_empty_keys()
            ]
    ...
    collision_rate_n = {
        a_id: np.mean(
            [
                path[a_id]["env_infos"][-1]["collision"]
                for path in paths
                if a_id in path.non_empty_keys()
            ]
        )
    ...
    rewards_n = {
        a_id: np.concatenate(
            [path[a_id]["rewards"] for path in paths if a_id in path.non_empty_keys()]
        )
        for a_id in agent_ids
    }
    actions_n = {
        a_id: [path[a_id]["actions"] for path in paths if a_id in path.non_empty_keys()]
        for a_id in agent_ids
    }
    ...
    statistics.update(
        create_stats_ordered_dict(
            "Ep. Len.",
            np.array([len(path) for path in paths]),
            stat_prefix=stat_prefix,
            always_show_all_stats=True,
        )
    )
    ...
    if dpo:
        for a_id in agent_ids:
            pred_next_obs = np.concatenate(
                [
                    path[a_id]["pred_observations"]
                    for path in paths
                    if a_id in path.non_empty_keys()
                ],
                axis=0,
...
def get_agent_mean_avg_returns(paths, std=False):
    returns = [
        np.mean([sum(path[a_id]["rewards"]) for a_id in path.non_empty_keys()])
        for path in paths
    ]
    if std:
