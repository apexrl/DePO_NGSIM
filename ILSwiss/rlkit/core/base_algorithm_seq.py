...
class BaseAlgorithm(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.possible_types = env.possible_vehicle_types
        if training_env is not None:
            self.training_env_num = {
                t: training_env[t].env_num for t in self.possible_types
            }
            self.training_env_wait_num = {
                t: training_env[t].wait_num for t in self.possible_types
            }
        ...
        self.observations_n = {
            t: self.training_env[t].reset() for t in self.possible_types
        }
        self.actions_n = {
            t: np.array(
                [
                    {
                        a_id: self.action_space_n[a_id].sample()
                        for a_id in self.agent_ids
                    }
                    for _ in range(self.training_env_num[t])
                ]
            )
        ...
        self._ready_env_ids = {
            t: np.arange(self.training_env_num[t]) for t in self.possible_types
        }
    ...
    def _can_train(self):
        return (
            self.replay_buffer.num_steps_can_sample(mode="max")
        ...
        for idx, observation_n in enumerate(observations_n):
            for agent_id in observation_n.keys():
                ...
                actions_n[idx][agent_id] = self.exploration_policy_n[
                    policy_id
                ].get_action(observation_n[agent_id])[0]
        return actions_n
    ...
    def _handle_vec_rollout_ending(self, end_idx):
        ...
        """
        for idx in end_idx:
            self._handle_path(self._current_path_builder[self.current_type][idx])
            self.replay_buffer.terminate_episode()
            self._n_rollouts_total += 1
            if len(self._current_path_builder[self.current_type][idx]) > 0:
                self._exploration_paths.append(
                    self._current_path_builder[self.current_type][idx]
                )
                self._current_path_builder[self.current_type][idx] = PathBuilder(
                    self.agent_ids
                )
    ...
    def get_extra_data_to_save(self, epoch):
        ...
        """
        if self.render:
            self.training_env[self.current_type].render(close=True)
        data_to_save = dict(
            epoch=epoch,
        )
        if self.save_environment:
    ...
    def evaluate(self, epoch, dpo=False, ego_obs_idx_n=None):
        """
