...
def experiment(variant):
    ...
    training_env = get_envs(
        ...
        vehicle_type_list=possible_vehicle_types
        * (env_specs["training_env_specs"]["env_num"] // num_vehicle_types),
    ...
    print(
        "Creating {} x {} = {} evaluation environments, each with {} vehicles ...".format(
            env_specs["eval_env_specs"]["env_num"],
            num_vehicle_types,
            env_specs["eval_env_specs"]["env_num"] * num_vehicle_types,
            len(eval_vehicle_ids_list[0]),
        )
    )
    env_specs["eval_env_specs"]["env_num"] *= num_vehicle_types
    env_specs["eval_env_specs"]["wait_num"] *= num_vehicle_types
    eval_env = get_envs(
        env_specs,
        env_wrapper,
        vehicle_ids_list=eval_vehicle_ids_list * num_vehicle_types,
        vehicle_type_list=possible_vehicle_types
        * (env_specs["eval_env_specs"]["env_num"] // num_vehicle_types),
        **env_specs["eval_env_specs"],
    )
    eval_car_num = np.array(
        [len(v_ids) for v_ids in eval_vehicle_ids_list] * num_vehicle_types
    )
