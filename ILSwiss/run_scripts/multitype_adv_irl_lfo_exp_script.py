...
from rlkit.torch.algorithms.adv_irl.adv_irl_lfo_seq import AdvIRL_LfO_Seq
...
def experiment(variant):
    ...
    print("\nSAMPLE NUM {}\n".format(variant["adv_irl_params"]["sample_num"]))

    if variant.get("share_state_predictor", False):
        shared_state_predictor = None

    if variant.get("share_discriminator", False):
        ...
        if policy_id not in policy_trainer_n:
            ...
            if variant.get("share_discriminator", False):
                if shared_disc_model is None:
                    ...
                    shared_disc_model = disc_model
                    print(f"Create discriminator in {policy_id} ...")
                else:
                    disc_model = shared_disc_model
                    print(f"Use shared discriminator in {policy_id} ...")
    ...
    env = env_wrapper(env)
    possible_vehicle_types = env.possible_vehicle_types
    num_vehicle_types = len(possible_vehicle_types)
    ...
    print(
        "Creating {} x {} = {} training environments, each with {} vehicles ...".format(
            env_specs["training_env_specs"]["env_num"],
            num_vehicle_types,
            env_specs["training_env_specs"]["env_num"] * num_vehicle_types,
            len(train_vehicle_ids_list[0]),
        )
    )
    training_env = {}
    for vehicle_type in possible_vehicle_types:
        training_env[vehicle_type] = get_envs(
            env_specs,
            env_wrapper,
            vehicle_ids_list=train_vehicle_ids_list,
            vehicle_type_list=np.array([vehicle_type]).repeat(
                (env_specs["training_env_specs"]["env_num"]),
            ),
            **env_specs["training_env_specs"],
        )
        env_specs["training_env_specs"]["seed"] += len(possible_vehicle_types)

    # env_specs["training_env_specs"]["env_num"] *= num_vehicle_types
    # env_specs["training_env_specs"]["wait_num"] *= num_vehicle_types
    # _training_env = get_envs(
    #     env_specs,
    #     env_wrapper,
    #     vehicle_ids_list=train_vehicle_ids_list * num_vehicle_types,
    #     vehicle_type_list=np.array(possible_vehicle_types).repeat(
    #         (env_specs["training_env_specs"]["env_num"] // num_vehicle_types),
    #     ),
    #     **env_specs["training_env_specs"],
    # )
    ...
    if env_specs.get("clip_action", False):
        from copy import copy

        env_specs = copy(env_specs)
        env_specs["clip_action"] = False
    print(
        "Creating {} evaluation environments, each with {} vehicles ...".format(
            env_specs["eval_env_specs"]["env_num"],
            len(eval_vehicle_ids_list[0]),
        )
    )
    env_specs["eval_env_specs"]["env_num"]
    env_specs["eval_env_specs"]["wait_num"]
    eval_env = get_envs(
        env_specs,
        env_wrapper,
        vehicle_ids_list=eval_vehicle_ids_list,
        vehicle_type_list=np.array(possible_vehicle_types[0]).repeat(
            (env_specs["eval_env_specs"]["env_num"]),
        ),
...
if __name__ == "__main__":
    ...
    if "share_discriminator" in exp_specs:
        exp_suffix += "--share_disc-{}".format(exp_specs["share_discriminator"])
